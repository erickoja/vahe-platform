...
function Dashboard({clients,jobs,quotes,payments,invoices,appointments=[],proposals=[],markProposalSeen,markRepairSeen,markupTable,biz,setBiz,setView,setSelClient,openJobs,spotPrices,onUpdateSpot}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(176px,1fr))",gap:14,marginBottom:24}}>
      ...
      <Stat label="This month" value={fmtR(monthReceived)} sub={monthTrend?`vs ${fmtR(lastMonthReceived)} last month`:"received (incl. trade-ins)"} tint="mint" icon={ICON_MONEY} trend={trendChip}/>
  ...
  return <div>
    ...
    {!isMobile&&<div style={{display:"flex",gap:6,marginBottom:14,alignItems:"center"}}>
      {[["list","☰ List"],["board","▦ Board"]].map(([m,label])=>(
        <button key={m} onClick={()=>setMode(m)} style={{padding:"7px 15px",borderRadius:8,border:`1px solid ${mode===m?GOLD:BD}`,background:mode===m?GOLD:WHITE,color:mode===m?WHITE:WG,fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit"}}>{label}</button>
      ))}
    ...
    {vMode==="list"&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
      {["All",...JOB_STAGES].map(s=>{const n=stageCount(s);return <button key={s} onClick={()=>setSf(s)} style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${sf===s?GOLD:BD}`,background:sf===s?GOLD:WHITE,color:sf===s?WHITE:WG,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{s}{n>0?<span style={{opacity:0.65,fontWeight:600}}> ({n})</span>:""}</button>;})}
    </div>}
    {vMode==="list"&&<div style={{marginBottom:14,display:"flex",gap:6,flexWrap:"wrap"}}>
      <button onClick={()=>setOverdueOnly(v=>!v)} style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${overdueOnly?DANGER:BD}`,background:overdueOnly?DANGER:WHITE,color:overdueOnly?WHITE:WG,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>⏰ Overdue</button>
...
function Reports({jobs,clients,quotes,payments,invoices,markupTable,setView}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(176px,1fr))",gap:12,marginBottom:22}}>
      ...
      <Stat label="Total sales" value={fmtR(totalSales)} sub="agreed charges" tint="slate" icon={ICON_MONEY}/>
      <Stat label="Quote conversion" value={`${conv}%`} sub={`${appQ} of ${totalQ} approved`} tint="mint" icon={ICON_CHECK}/>
      <Stat label="Avg base cost" value={fmtR(avgBase)} tint="slate" icon={ICON_DOLLAR}/>
